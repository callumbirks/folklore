use core::{borrow::Borrow, cell::Cell, hash::Hash, mem::size_of, sync::atomic::AtomicU16};
...
// How many insertions on each thread before we update count with the thread's local counter.
// Higher number is lower frequency.
const COUNT_UPDATE_FREQ: Size = 6;
    ...
    V: Copy + NoUninit,
{
    ...
    pub fn insert(&self, key: K, value: V) -> bool {
        if self.estimate_count() >= self.capacity {
            return false;
        }
...
#[allow(clippy::cast_possible_truncation)]
const fn wrap_hash(hash: u32, size: Size) -> Size {
    (hash % size as u32) as Size
}

const fn wrap_index(index: Size, size: Size) -> Size {
    index % size
}
